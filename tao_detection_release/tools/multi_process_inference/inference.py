...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='./configs/tao/gs_htc_dconv_c3-c5_mstrain_400_1400_x101_64x4d_fpn_20e_lvis.py', help='test config file path')
    parser.add_argument('--checkpoint', type=str, default='./work_dirs/bags/gs_htc_dconv_c3-c5_mstrain_400_1400_x101_64x4d_fpn_20e_lvis.pth', help='checkpoint file')
    parser.add_argument('--img_file', type=str, default='', help='inference img file')
    parser.add_argument('--img_root', type=str, default='./data/tao/frames/', help='inference img dir')
    ...
    parser.add_argument('--split', type=str, default='train', help='output dir')
...
def main():
    ...
    if args.img_file != '':
        file_list = [args.img_file]
    elif  args.img_list != '':
        file_list = parse_textfile(args.img_list)
    else:
        raise "Both img_file and img_list is empty."

    if args.img_root != '':
        file_list = [os.path.join(args.img_root, file) for file in file_list]
    ...
    for i, data in enumerate(data_loader):
        ...
        save_name = file_path.replace(f'{args.img_root}', '') 
        save_name = save_name.replace('.jpg', '.pkl').replace('.jpeg', '.pkl')
